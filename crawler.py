...
class Crawler():
    ...
    def __init__(self, url, depth_limit, db, key, logger, tp, lock):
        ...
        self.cur_failed = 0  # 当前层访问失败的URL总数
    ...
    def crawl(self, url, depth):
        ...
        if depth > self.depth_limit:  # 当前深度超过指定深度, 停止抓取, 将工作队列标记为任务完成
            self.db.conn.commit()
            self.logger.info('save content to database table: {}'.format(self.table_name))
        ...
        if depth > self.cur_level:  # 进入下一层
            ...
            time.sleep(1 * self.cur_level * self.tp.thread_num)
            with self.lock:
    ...
    def change_level(self, depth):
        ...
        self.logger.info('level {} finished, total URL number: {}, failed {}'.format(
            self.cur_level, self.cur_total, self.cur_failed))
        print('\n' + '=' * 60)
        self.total_finished += self.tp.cur_finished # 更新完成总数
        self.tp.cur_finished = 0  # 当前层已完成任务数重置为0
        self.total_failed += self.cur_failed # 更新失败总数
        self.cur_failed = 0 # 当前层失败数置0
        self.total += self.cur_total # 更新URL总数
        self.cur_total = self.next_total  # 设置新一层的任务总数, 此数目由爬取完上层URL之后得来
        self.next_total = 0 # 下层总数置0
        self.cur_level = depth # 当前层编号设置为下层编号
        self.logger.info('enter the {} level'.format(self.cur_level))
        self.logger.info('the URL numbers of the {} level is {}'.format(
            self.cur_level, self.cur_total))
    ...
    def get_urls_from_url(self, url):
        ...
        try:
            response = requests.get(url, headers=headers, timeout=2) # 可视网络情况调整 timeout 时长
    ...
    def report(self):
        ...
        time.sleep(5)
        print('\n','-' * 30,' 结果统计 ', '-' * 30)
        print('总URL数: {}, 共抓取: {}, 其中失败: {}'.format(self.total, self.total_finished, self.total_failed))
        try:
            num = self.db.curs.execute("SELECT COUNT(*) AS NUM FROM {}".format(self.table_name))
...
class Progress(threading.Thread):
    ...
    def show_progress(self):
        ...
        sys.stdout.write('当前层: {}, 该层URL总数: {}, 已完成: {}, 失败: {}, 进度: {:.2f}%\r'.format(
            self.c.cur_level, self.c.cur_total, self.tp.cur_finished,
            self.c.cur_failed, self.tp.cur_finished / self.c.cur_total * 100))
        sys.stdout.flush()
...
class Worker(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            self.tp.cur_finished += 1  # 更新完成任务数
...
class Threadpool():
    ...
    def __init__(self, thread_num):
        ...
        self.threads = []  # 线程池
        self.cur_finished = 0 # 当前层完成任务数初始化为0
...
def main():
    ...
    try:
        ...
        c.total += c.cur_total # 任务完成后更新完成总数, 用于报告
        c.total_failed += c.cur_failed
        c.total_finished += tp.cur_finished
